...
function renderHero() {
  ...
  $("#snapshot-list").innerHTML = DATA.snapshot.map(i => `<li><span class="snap-k">${i.label}</span><span class="snap-v">${i.value}</span></li>`).join("");
  $("#fast-review").innerHTML = DATA.fastReview.map(i => `<a href="${i.target}">${i.label}</a>`).join("");
  $("#metrics").innerHTML = DATA.metrics.map(m => `<a class="m-card" href="${m.target}"><strong>${m.value}</strong><span>${m.label}</span></a>`).join("");
...
function renderPublications(filter = "all") {
  ...
  $("#pub-list").innerHTML = rows.map(p => `
    <article class="pub-card">
      ...
      <h3>${p.title}</h3>
      <p class="pub-authors">${p.authors} (${p.year}). <strong>${p.venue}</strong>.</p>
      ${p.doi ? `<p class="doi-line">DOI: <a href="${p.doiUrl}" target="_blank" rel="noopener">${p.doi} ↗</a></p>` : ""}
    </article>
  `).join("");
}
...
/* V91: aggressively suppress Google Translate banner/ribbon after language changes */
(function(){
  function removeGoogleTranslateChrome(){
    ...
    document.querySelectorAll(
      ...
      "body > .skiptranslate, iframe.skiptranslate, .VIpgJd-ZVi9od-ORHb, .VIpgJd-ZVi9od-ORHb-OEVmcd, " +
      ".VIpgJd-yAWNEb-L7lbkb, .goog-tooltip, .goog-tooltip:hover, #goog-gt-tt"
    ).forEach(function(el){
  ...
  window.addEventListener("load", removeGoogleTranslateChrome);
  window.addEventListener("resize", removeGoogleTranslateChrome);
  document.addEventListener("change", function(event){
    if (event.target && event.target.classList && event.target.classList.contains("goog-te-combo")) {
      setTimeout(removeGoogleTranslateChrome, 80);
      setTimeout(removeGoogleTranslateChrome, 500);
      setTimeout(removeGoogleTranslateChrome, 1200);
    }
  });

  if (document.body) {
    new MutationObserver(removeGoogleTranslateChrome).observe(document.body, {
      childList:true,
      subtree:true
    });
  }
...
/* V96: clean inline Default English link inside the language bar */
(function(){
  function resetGoogleTranslateCookies(){
    var past = "Thu, 01 Jan 1970 00:00:00 GMT";
    var hosts = [location.hostname, "." + location.hostname];
  ...
  function bindInlineEnglishLink(){
    var link = document.getElementById("language-default-en");
    if (!link || link.dataset.boundV96 === "1") return;
    link.dataset.boundV96 = "1";
  ...
  function boot(){
    bindInlineEnglishLink();
    var tries = 0;
    var timer = setInterval(function(){
      tries += 1;
      if (renamePlaceholder() || tries > 30) clearInterval(timer);
    }, 250);
  }
...
/* V99: V97-safe anchor scroll offset only; layout untouched */
(function(){
  function headerOffset(){
    ...
    return headerHeight + 28; /* one-line visual gap */
  }
  ...
  function bindAnchorScrolling(){
    document.querySelectorAll('a[href^="#"]').forEach(function(link){
      if (link.dataset.v99ScrollBound === "1") return;
      link.dataset.v99ScrollBound = "1";
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bindAnchorScrolling);
  } else {
    bindAnchorScrolling();
  }
})();
