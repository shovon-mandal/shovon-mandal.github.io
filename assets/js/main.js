...
function renderHero() {
  ...
  /* Green sliding line under "Academic Research Goal" (original fade transition, every 2.6 s) */
  let words = (s.rotatingWords || []).filter(Boolean);
  if (!words.length && s.showRotatingLines !== false) words = (FALLBACK_DATA.site.rotatingWords || []);
  if (s.showRotatingLines === false) words = [];
  const gr = $("#goal-rotate");
  gr.hidden = !words.length;
  gr.innerHTML = words.map((w, i) => `<span class="${i === 0 ? "active" : ""}">${w}</span>`).join("");
  if (words.length > 1) {
    setInterval(() => {
  ...
  /* Latest updates ticker (same slide transition), from the Updates list */
  const news = (DATA.news || []).filter(n => n && n.text);
  const hu = $("#hero-updates");
  if (hu) {
    hu.hidden = !news.length || s.showHeroUpdates === false;
    setupRotator($("#hero-updates-rotate"), news.map(n => `${n.date ? `<b>${n.date}</b>` : ""}${n.text}`), 4200);
  }
}

/* Slide-up text rotator with a thin timing bar; pauses on hover; respects reduced motion */
function setupRotator(box, items, interval) {
  if (!box) return;
  box.classList.add("rotator");
  box.innerHTML = items.map((w, i) => `<span class="${i === 0 ? "active" : ""}">${w}</span>`).join("") +
    (items.length > 1 ? `<i class="rotator-bar" aria-hidden="true"></i>` : "");
  box.style.setProperty("--rot-ms", interval + "ms");
  if (items.length < 2) return;
  let idx = 0, paused = false;
  const host = box.closest(".goal-box, .hero-updates") || box;
  host.addEventListener("mouseenter", () => { paused = true; box.classList.add("paused"); });
  host.addEventListener("mouseleave", () => { paused = false; box.classList.remove("paused"); });
  const restartBar = () => {
    const bar = box.querySelector(".rotator-bar");
    if (!bar) return;
  ...
  setInterval(() => {
    ...
    setTimeout(() => cur.classList.remove("leaving"), 650);
    restartBar();
  }, interval);
}
...
function renderContact() {
  ...
  if (!c) return;
  if (c.lead && $("#contact-lead")) $("#contact-lead").textContent = c.lead;
  const list = $("#contact-list");
  if (!list || !(c.cards || []).length) return;
  list.innerHTML = c.cards.map(k => {
    const ext = /^https?:/i.test(k.url || "") ? ' target="_blank" rel="noopener"' : "";
    return `<a class="final-contact-card" href="${k.url || "#contact"}"${ext}>
      <span class="final-contact-icon">${k.icon || ""}</span>
      <span class="final-contact-copy"><strong>${k.title || ""}</strong><em>${k.text || ""}</em></span>
    </a>`;
  }).join("");
}
...
/* Publication filters: show how many papers are in each group, hide empty ones */
function hideEmptyPubFilters() {
  const pubs = PortfolioCore.websitePubs(DATA);
  $$('.filt[data-scope="pub"]').forEach(btn => {
    const f = btn.dataset.filter;
    if (!btn.dataset.label) btn.dataset.label = btn.textContent.trim();
    const n = f === "all" ? pubs.length : pubs.filter(p => pubHaystack(p).includes(f.toLowerCase())).length;
    btn.hidden = f !== "all" && n === 0;
    btn.innerHTML = `${btn.dataset.label}<span class="filt-count">${n}</span>`;
    btn.setAttribute("aria-label", `${btn.dataset.label}: ${n} ${n === 1 ? "paper" : "papers"}`);
  });
}
...
function setupNavigation() {
  ...
  }, { threshold: 0, rootMargin: "-40% 0px -55% 0px" });   /* "active" = section crossing the middle of the screen, works for very tall sections too */
...
/* Fade-in sections. Uses "any part visible" (threshold 0) so very tall sections,
   like Publications on a phone, always appear. Safety net: show everything after 2.5 s. */
function setupReveal() {
  ...
  items.forEach(x => reveal.observe(x));
  setTimeout(() => items.forEach(x => x.classList.add("visible")), 2500);
}
...
/* "Updated 24 Sep 2026" under the CV buttons, same date as the PDF stamp */
function showCvDate() {
  if (!UPDATED_AT) return;
  const d = new Date(UPDATED_AT);
  if (isNaN(d.getTime())) return;
  const m = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][d.getMonth()];
  const text = `Updated ${d.getDate()} ${m} ${d.getFullYear()}`;
  ["#cv-updated", "#sb-cv-date"].forEach(sel => {
    const el = $(sel);
