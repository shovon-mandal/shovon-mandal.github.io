...
/* V94: safe guards so the portfolio content remains visible even if a browser blocks scripts */
(function(){
  document.documentElement.classList.add("portfolio-static-ready");

  function safeQuery(selector){ return document.querySelector(selector); }
  function safeAll(selector){ return Array.from(document.querySelectorAll(selector)); }
  ...
  function bindDefaultEnglish(){
    var link = document.getElementById("language-default-en");
    if (!link || link.dataset.boundV94 === "1") return;
    link.dataset.boundV94 = "1";
    link.addEventListener("click", function(event){
      event.preventDefault();
      resetGoogleTranslateCookies();
      var combo = document.querySelector(".goog-te-combo");
  ...
  function showAllContent(){
    safeAll(".reveal").forEach(function(el){ el.classList.add("visible"); });
    safeAll(".sec, .hero, .metrics, .paired-sections").forEach(function(el){
